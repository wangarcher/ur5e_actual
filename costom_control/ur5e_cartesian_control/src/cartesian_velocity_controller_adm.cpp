...
#include "cartesian_velocity_controller_adm.h"
 ...
 */
template<typename T>
bool CartesianVelocityControllerADMBase<T>::init(
  ...
  realtime_pub_.reset(new realtime_tools::RealtimePublisher
                      <ur5e_admittance_control::ADMPoseTwist>(n, "ee_state", 1));


  // Topics
  sub_command_ = n.subscribe("command_cart_vel", 1,
                         &CartesianVelocityControllerADMBase<T>::command_cart_vel,
 ...
 */
template<typename T>
void CartesianVelocityControllerADMBase<T>::starting(const ros::Time& time){
 ...
 */
template<typename T>
void CartesianVelocityControllerADMBase<T>::update(const ros::Time& time,
 ...
 */
template<typename T>
void CartesianVelocityControllerADMBase<T>::command_cart_vel(
 ...
 */
void CartesianVelocityControllerADM::writeVelocityCommands(
                                    const ros::Duration& period) {
    for(std::size_t i=0; i < this->joint_handles_.size(); i++) {
 ...
 * \param period The duration of an update cycle
 */
void CartesianVelocityControllerADMSim::writeVelocityCommands(
                                    const ros::Duration& period) {
  for(std::size_t i=0; i < this->joint_handles_.size(); i++) {
...
PLUGINLIB_EXPORT_CLASS(controller_interface::CartesianVelocityControllerADM,
                       controller_interface::ControllerBase)
PLUGINLIB_EXPORT_CLASS(controller_interface::CartesianVelocityControllerADMSim,
                       controller_interface::ControllerBase)
